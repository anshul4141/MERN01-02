...
const Users = () => {
    ...
    const handleUpdateUser = async () => {
        try {
            const response = await axios.put(`http://localhost:5000/user/updateUser/${selectedUser._id}`, formData);
            if (response.data.success) {
    ...
    const handleDelete = async (userId) => {
        ...
        if (confirmDelete) {
            try {
                const response = await axios.delete(`http://localhost:5000/user/deleteUser/${userId}`);
                if (response.data.success) {
